...
def timewarp(mel_spectrogram, time_warping_para=80):
    ...
    # print(warped_mel_spectrogram_op.shape)
    # print(mel_spectrogram.shape)
    # Change warp result's data type to numpy array for masking step.
    feed_dict = {mel_spectrogram_holder:mel_spectrogram,
                 location_holder:control_point_locations,
...
def freqmask(mel_spectrogram, frequency_masking_para=27, frequency_mask_num=1):
    v = mel_spectrogram.shape[0]           
    warped_mel_spectrogram = mel_spectrogram.copy()
    for i in range(frequency_mask_num):
        f = np.random.uniform(low=0.0, high=frequency_masking_para)
        f = int(f)
        f0 = random.randint(0, v - f)
...
def timemask(mel_spectrogram, time_warping_para=80, time_masking_para=100, time_mask_num=1):
    tau = mel_spectrogram.shape[1]
    # Change warp result's data type to numpy array for masking step.
    # v = mel_spectrogram.shape[0]
    # mel_spectrogram_holder = tf.placeholder(tf.float32, shape=[1, v, tau, 1])
    ...
    # with tf.Session() as sess:
    #     warped_mel_spectrogram = sess.run(mel_spectrogram, feed_dict=feed_dict)
